...
```mermaid
graph TD
    A[You via AWS Console / Session Manager] --> B[Ubuntu Controller (Ansible)]
    B --> C1[Ubuntu Client 1 - Ubuntu 22.04]
    B --> C2[Ubuntu Client 2 - Ubuntu 22.04]
    B --> D1[Amazon Linux Client 1 - AMZ 2023]
    B --> D2[Amazon Linux Client 2 - AMZ 2023]
...
- share the public key with the cleints but pasting it into authorised key file on each cleint and test the connectivity from the controller.
...
- verify if all setup is correct using the ad-hoc cmd ping module (ansible all -m ping)
...
- using the comman ansible-playbook multi_play.yml
